...
async function claimChannel(request: Request, ctx: RequestContext): Promise<Response> {
  ...
  const result = await new PreviewFeedbackChannelStore(ctx.db).claim({
    channelKey,
...
async function getChannel(request: Request, ctx: RequestContext): Promise<Response> {
  const body = await boundedBody<GetBody>(request);
  if (body instanceof Response) return body;
  const channelKey = requiredString(body.channelKey, 1000);
  if (!channelKey) return error("channelKey is required", 400);
  const channel = await new PreviewFeedbackChannelStore(ctx.db).get(channelKey);
...
async function updateChannel(request: Request, ctx: RequestContext): Promise<Response> {
  ...
  const channel = await new PreviewFeedbackChannelStore(ctx.db).update({
    channelKey,
    leaseOwner,
    now: body.now,
...
export const previewFeedbackChannelRoutes = new Hono<ControlPlaneHonoEnv>();

previewFeedbackChannelRoutes.post(
  "/preview-feedback/channels/claim",
  LINEAR_SERVICE,
  (c) => dispatch(c, (request, _env, _params, ctx) => claimChannel(request, ctx))
);
previewFeedbackChannelRoutes.post(
  "/preview-feedback/channels/get",
  LINEAR_SERVICE,
  (c) => dispatch(c, (request, _env, _params, ctx) => getChannel(request, ctx))
);
previewFeedbackChannelRoutes.post(
